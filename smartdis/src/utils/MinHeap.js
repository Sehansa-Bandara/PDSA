...
export class MinHeap {
   ...
   * 3. If product is expiring soon (<= 30 days):
   *    - Dairy: 60%
   *    - Meat: 50%
   *    - Others: Tiered based on days remaining
   * 4. If expired (< 0 days): 80% (Clearance)
   */
  static computeDiscount(days, category = 'Other', productManual = null, categoryManual = null) {
    ...
    // 2. Category Manual override (Only apply if expiring within 30 days)
    if (categoryManual !== null && categoryManual !== undefined && categoryManual !== '' && days <= 30) {
      return Number(categoryManual);
    }

    // 3. Expired clearance (optional: keep or remove? User said "dont add discount" so I'll remove it too)
    // if (days < 0) return 80;
